...
public:
  ...
  QString getValue(const QString &key) const override { return data.value(key); }
...
private Q_SLOTS:
  ...
  void testNotNodeEvaluate() {
    ...
    QSharedPointer<ASTNode> childTrue = QSharedPointer<InNode>::create(QStringLiteral("state"), QStringLiteral("open"));
    NotNode notNodeTrue(childTrue);
    QVERIFY(!notNodeTrue.evaluate(accessor));

    // Child evaluates to false, NotNode evaluates to true
    QSharedPointer<ASTNode> childFalse =
        QSharedPointer<InNode>::create(QStringLiteral("state"), QStringLiteral("closed"));
    ...
    QSharedPointer<ASTNode> doubleNot = QSharedPointer<NotNode>::create(QSharedPointer<NotNode>::create(childTrue));
    QVERIFY(doubleNot->evaluate(accessor));
    QCOMPARE(doubleNot->toString(), QStringLiteral("NOT NOT state IN \"open\""));
  }
  ...
  void testOrNodeEvaluate() {
    ...
    // Test toString serialization
    QCOMPARE(multipleTrueNode.toString(), QStringLiteral("(state:open OR author:jules)"));
  }
  ...
  void testInNodeEvaluate() {
    ...
    // No match
    InNode noMatchNode(QStringLiteral("state"), QStringLiteral("closed,merged"));
    QVERIFY(!noMatchNode.evaluate(accessor));

    // Missing key
    InNode missingKeyNode(QStringLiteral("missing"), QStringLiteral("open,closed"));
    QVERIFY(!missingKeyNode.evaluate(accessor));

    // Whitespace handling (should match because of trimming)
    InNode whitespaceNode(QStringLiteral("author"), QStringLiteral(" alice , jules , bob "));
    ...
    InNode secondMatchNode(QStringLiteral("state"), QStringLiteral("closed,open"));
  ...
  void testKeyValueNodeDateFilterEvaluate() {
    MockAccessor accessor;
    accessor.data.insert(QStringLiteral("createdat"), QStringLiteral("2023-01-15T12:00:00Z"));
    accessor.data.insert(QStringLiteral("updatedat"), QStringLiteral("2023-02-15T12:00:00Z"));

    // Happy path: created-before
    KeyValueNode createdBeforeNode(QStringLiteral("created-before"), QStringLiteral("2023-01-20T12:00:00Z"));
    QVERIFY(createdBeforeNode.evaluate(accessor));

    // Timezone handling: filter with offset (14:00+01:00 is 13:00Z, which is
    // after 12:00Z)
    KeyValueNode offsetNode(QStringLiteral("created-before"), QStringLiteral("2023-01-15T14:00:00+01:00"));
    QVERIFY(offsetNode.evaluate(accessor));

    // created-before fails
    KeyValueNode createdBeforeNodeFail(QStringLiteral("created-before"), QStringLiteral("2023-01-10T12:00:00Z"));
    QVERIFY(!createdBeforeNodeFail.evaluate(accessor));

    // Happy path: created-after
    KeyValueNode createdAfterNode(QStringLiteral("created-after"), QStringLiteral("2023-01-10T12:00:00Z"));
    QVERIFY(createdAfterNode.evaluate(accessor));

    // created-after fails
    KeyValueNode createdAfterNodeFail(QStringLiteral("created-after"), QStringLiteral("2023-01-20T12:00:00Z"));
    QVERIFY(!createdAfterNodeFail.evaluate(accessor));

    // Happy path: updated-before
    KeyValueNode updatedBeforeNode(QStringLiteral("updated-before"), QStringLiteral("2023-02-20T12:00:00Z"));
    QVERIFY(updatedBeforeNode.evaluate(accessor));

    // Happy path: updated-after
    KeyValueNode updatedAfterNode(QStringLiteral("updated-after"), QStringLiteral("2023-02-10T12:00:00Z"));
    QVERIFY(updatedAfterNode.evaluate(accessor));

    // Exact same date should return false
    KeyValueNode exactDateNode(QStringLiteral("created-before"), QStringLiteral("2023-01-15T12:00:00Z"));
    QVERIFY(!exactDateNode.evaluate(accessor));

    // Exact same date for after filter should also return false
    KeyValueNode exactDateAfterNode(QStringLiteral("created-after"), QStringLiteral("2023-01-15T12:00:00Z"));
    QVERIFY(!exactDateAfterNode.evaluate(accessor));

    // updated-before fails
    KeyValueNode updatedBeforeNodeFail(QStringLiteral("updated-before"), QStringLiteral("2023-02-10T12:00:00Z"));
    QVERIFY(!updatedBeforeNodeFail.evaluate(accessor));

    // Invalid date in filter
    KeyValueNode invalidFilterDateNode(QStringLiteral("created-before"), QStringLiteral("not-a-date"));
    QVERIFY(!invalidFilterDateNode.evaluate(accessor));

    // Invalid date in data
    MockAccessor invalidDataAccessor;
    invalidDataAccessor.data.insert(QStringLiteral("createdat"), QStringLiteral("invalid-date"));
    KeyValueNode validFilterInvalidDataNode(QStringLiteral("created-before"), QStringLiteral("2023-01-20T12:00:00Z"));
  ...
  void testApplyQuickFilter() {
    ...
    QString updated = FilterEditor::applyQuickFilter(base, QStringLiteral("owner"), QStringLiteral("jules"), false);
    ...
    QString hideUpdated = FilterEditor::applyQuickFilter(base, QStringLiteral("owner"), QStringLiteral("jules"), true);
  ...
  void testMergeFilterIntoAST() {
    ...
    QString hideUpdated = FilterEditor::applyQuickFilter(base, QStringLiteral("repo"), QStringLiteral("other"), true);
    // It should convert it to an OR inside the NOT
    QVERIFY(hideUpdated.contains(QStringLiteral("NOT (repo:test OR repo:other)")));
  }
