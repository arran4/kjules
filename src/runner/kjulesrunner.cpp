...
KJulesRunner::KJulesRunner(QObject *parent, const KPluginMetaData &metaData, const QVariantList &args)
...
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
    addSyntax(KRunner::RunnerSyntax(QStringLiteral("kjules :q:"), i18n("Create a new kjules session with the given prompt.")));
#else
    addSyntax(Plasma::RunnerSyntax(QStringLiteral("kjules :q:"), i18n("Create a new kjules session with the given prompt.")));
#endif
}
...
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
void KJulesRunner::run(const KRunner::RunnerContext &context, const KRunner::QueryMatch &match) {
#else
void KJulesRunner::run(const Plasma::RunnerContext &context, const Plasma::QueryMatch &match) {
#endif
    Q_UNUSED(context);
