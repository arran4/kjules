...
BackupDialog::BackupDialog(const QString &defaultDir, QWidget *parent) : QDialog(parent) {
  ...
  // Explanatory text
  QLabel *infoLabel = new QLabel(i18n("This will archive your local data into a zip file. "
  ...
  connect(browseBtn, &QPushButton::clicked, this, &BackupDialog::browseDirectory);
  ...
  // Options
  m_removeCheckBox = new QCheckBox(i18n("Remove original files after backup"), this);
  m_removeCheckBox->setChecked(false); // Dangerous option, default off
  mainLayout->addWidget(m_removeCheckBox);

  m_openDirCheckBox = new QCheckBox(i18n("Open directory containing backup afterwards"), this);
...
void BackupDialog::browseDirectory() {
  QString dir = QFileDialog::getExistingDirectory(this, i18n("Select Backup Directory"), m_dirEdit->text());
  if (!dir.isEmpty()) {
...
bool BackupDialog::removeOriginals() const { return m_removeCheckBox->isChecked(); }

bool BackupDialog::openDirectory() const { return m_openDirCheckBox->isChecked(); }
