...
NewSessionDialog::NewSessionDialog(SourceModel *sourceModel, QWidget *parent)
  ...
  // Load persisted mode
  KConfigGroup config(KSharedConfig::openConfig(), "NewSession");
  QString lastMode = config.readEntry("LastAutomationMode", QString());
  int index = m_automationModeCombo->findData(lastMode);
  if (index != -1) {
...
void NewSessionDialog::onSubmit() {
  ...
  // Save mode
  KConfigGroup config(KSharedConfig::openConfig(), "NewSession");
  config.writeEntry("LastAutomationMode", automationMode);
...
void NewSessionDialog::closeEvent(QCloseEvent *event) {
  if (m_canClose) {
    event->accept();
    return;
  }
  ...
  QMessageBox::StandardButton reply = QMessageBox::question(
      this, tr("Unsaved Draft"), tr("Do you want to save this draft?"),
