      ...
      [this](const QPoint &pos) {
        ...
        if (!owner.isEmpty() && !repo.isEmpty()) {
          ...
          connect(hideRepoAction, &QAction::triggered, [this, repo]() {
            m_filterEditor->setText(FilterEditor::applyQuickFilter(
                m_filterEditor->text(), QStringLiteral("repo"), repo, true));
            applyFilter();
          });
          connect(onlyRepoAction, &QAction::triggered, [this, repo]() {
            m_filterEditor->setText(FilterEditor::applyQuickFilter(
                m_filterEditor->text(), QStringLiteral("repo"), repo, false));
            applyFilter();
          });
          connect(hideOwnerAction, &QAction::triggered, [this, owner]() {
            m_filterEditor->setText(FilterEditor::applyQuickFilter(
                m_filterEditor->text(), QStringLiteral("owner"), owner, true));
            applyFilter();
          });
          connect(onlyOwnerAction, &QAction::triggered, [this, owner]() {
            m_filterEditor->setText(FilterEditor::applyQuickFilter(
                m_filterEditor->text(), QStringLiteral("owner"), owner, false));
            applyFilter();
          });
          menu.addSeparator();
        }
        ...
        if (rawData.contains(QStringLiteral("github"))) {
          ...
          if (!language.isEmpty()) {
            ...
            connect(hideLanguageAction, &QAction::triggered,
                    [this, language]() {
                      m_filterEditor->setText(FilterEditor::applyQuickFilter(
                          m_filterEditor->text(), QStringLiteral("language"),
                          language, true));
                      applyFilter();
                    });
