...
public:
    ...
    QString getValue(const QString &key) const override {
        ...
        for (int c = 0; c < model->columnCount(parent); ++c) {
            QString header = model->headerData(c, Qt::Horizontal, Qt::DisplayRole).toString().remove(QLatin1Char(' ')).toLower();
            if (header == key.toLower() || model->headerData(c, Qt::Horizontal, Qt::DisplayRole).toString().toLower() == key.toLower()) {
