      ...
      m_refreshTimer(new QTimer(this))
{
    ...
    connect(m_apiManager, &APIManager::sourcesReceived, m_sourceModel, &SourceModel::setSources);
...
void MainWindow::setupUi()
{
    ...
    connect(m_sessionView, &QListView::customContextMenuRequested, [this](const QPoint &pos) {
    ...
    connect(m_sessionView, &QListView::doubleClicked, this, &MainWindow::onSessionActivated);
    ...
    connect(m_draftsView, &QListView::customContextMenuRequested, [this](const QPoint &pos) {
        ...
        if (index.isValid()) {
            ...
            connect(submitAction, &QAction::triggered, [this, index]() {
                onDraftActivated(index);
            });
            ...
            connect(deleteAction, &QAction::triggered, [this, index]() {
                 if (QMessageBox::question(this, i18n("Delete Draft"), i18n("Are you sure?")) == QMessageBox::Yes) {
...
void MainWindow::setupTrayIcon()
{
    ...
    connect(m_trayIcon, &KStatusNotifierItem::activateRequested, this, &MainWindow::toggleWindow);
}
...
void MainWindow::onSessionCreated(const QStringList &sources, const QString &prompt, const QString &automationMode)
{
...
void MainWindow::onDraftActivated(const QModelIndex &index)
{
    ...
    connect(&dialog, &NewSessionDialog::saveDraftRequested, [this, index](const QJsonObject &d) {
        m_draftsModel->removeDraft(index.row());
