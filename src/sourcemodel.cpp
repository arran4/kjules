...
QString SourceModel::getDefaultBranch(const QString &sourceId) const {
  ...
  for (int i = 0; i < m_sources.size(); ++i) {
    ...
    if (normalizeSourceId(currentId) == normId) {
      ...
      if (source.contains(QStringLiteral("githubRepo"))) {
        QJsonObject githubRepo =
            source.value(QStringLiteral("githubRepo")).toObject();
        if (githubRepo.contains(QStringLiteral("defaultBranch"))) {
          QString julesBranch = githubRepo.value(QStringLiteral("defaultBranch"))
                                    .toObject()
