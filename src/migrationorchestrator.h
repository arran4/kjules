...
public:
  ...
  static void executeMigrationIfNecessary() {
      if (getMigratedFlag()) return;

      QString destinationPath = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + QStringLiteral("/jobs.json");
...
static bool safeMigrationSeam(const LegacyData &legacyData, const QString &destinationStorePath,
    ...
    if (!result.unattachedErrors.isEmpty()) return false;
    ...
    if (result.jobs.isEmpty() && legacyData.queueItems.isEmpty() && legacyData.activeSessions.isEmpty() &&
      ...
      if (emptyStore.save()) return true;
      return false;
    }
    ...
    QJsonArray arr1;
    for (auto &j : result.jobs) arr1.append(j.toJson());
    QJsonArray arr2;
    for (auto &j : validationStore.jobs()) arr2.append(j.toJson());
