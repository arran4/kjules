...
struct Token {
    enum Type {
        LPAREN, RPAREN, AND, OR, NOT, IN, KV, STR, WORD
    };
    Type type;
    QString val1;
    QString val2;
...
static QList<Token> tokenize(const QString &query) {
    ...
    while (i.hasNext()) {
        ...
        if (!match.captured(1).isEmpty()) {
            ...
            if (op == QStringLiteral("OR")) tokens.append({Token::OR, op, QString()});
            else if (op == QStringLiteral("AND")) tokens.append({Token::AND, op, QString()});
            else if (op == QStringLiteral("NOT")) tokens.append({Token::NOT, op, QString()});
            else if (op == QStringLiteral("IN")) tokens.append({Token::IN, op, QString()});
...
private:
    ...
    QSharedPointer<ASTNode> parseAnd() {
        ...
        while (!atEnd() && current().type != Token::OR && current().type != Token::RPAREN) {
...
bool KeyValueNode::evaluate(const FilterDataAccessor &accessor) const {
    ...
    if (lowerKey == QStringLiteral("created-before") || lowerKey == QStringLiteral("updated-before") ||
        ...
        QString dateStr;
        if (lowerKey.startsWith(QStringLiteral("created"))) dateStr = accessor.getValue(QStringLiteral("createdat"));
        else dateStr = accessor.getValue(QStringLiteral("updatedat"));
...
QString KeyValueNode::toString() const {
    if (m_value.contains(QLatin1Char(' '))) return m_key + QStringLiteral(":") + QStringLiteral("\"") + m_value + QStringLiteral("\"");
