...
QJsonObject JobAttemptData::toJson() const {
    ...
    if (createdAt.isValid()) obj[QStringLiteral("createdAt")] = createdAt.toString(Qt::ISODate);
    if (updatedAt.isValid()) obj[QStringLiteral("updatedAt")] = updatedAt.toString(Qt::ISODate);
    if (!launchErrors.isEmpty()) obj[QStringLiteral("launchErrors")] = launchErrors;
    if (!rawResponse.isEmpty()) obj[QStringLiteral("rawResponse")] = rawResponse;
    if (!prMetadata.isEmpty()) obj[QStringLiteral("prMetadata")] = prMetadata;
...
QJsonObject JobData::toJson() const {
    ...
    if (createdAt.isValid()) obj[QStringLiteral("createdAt")] = createdAt.toString(Qt::ISODate);
    if (updatedAt.isValid()) obj[QStringLiteral("updatedAt")] = updatedAt.toString(Qt::ISODate);
    if (!lifecycleMetadata.isEmpty()) obj[QStringLiteral("lifecycleMetadata")] = lifecycleMetadata;
    QJsonArray attemptsArray;
    for (const auto& attempt : attempts) {
        attemptsArray.append(attempt.toJson());
    }
    obj[QStringLiteral("attempts")] = attemptsArray;
    if (!acceptedAttemptId.isEmpty()) obj[QStringLiteral("acceptedAttemptId")] = acceptedAttemptId;
    if (!legacyMetadata.isEmpty()) obj[QStringLiteral("legacyMetadata")] = legacyMetadata;
    return obj;
}
