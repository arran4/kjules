    ...
    if (reply->error() == QNetworkReply::NoError) {
      ...
      if (doc.object().contains("sources")) {
        ...
        emit connectionTested(
            false,
            "Connection successful but no sources found or invalid response.");
      }
    } else {
      emit connectionTested(false,
                            "Connection failed: " + reply->errorString());
    }
    reply->deleteLater();
  });
}
...
void APIManager::listSources(const QString &pageToken) {
  QString endpoint = "/sources";
  if (!pageToken.isEmpty()) {
    endpoint += "?pageToken=" + pageToken;
  }
  QNetworkRequest request = createRequest(endpoint);
  QNetworkReply *reply = m_nam->get(request);
  connect(reply, &QNetworkReply::finished, this, [this, reply, pageToken]() {
    if (reply->error() == QNetworkReply::NoError) {
      QByteArray data = reply->readAll();
      QJsonDocument doc = QJsonDocument::fromJson(data);
...
void APIManager::getSession(const QString &sessionId) {
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply]() {
    if (reply->error() == QNetworkReply::NoError) {
      ...
      emit errorOccurred("Failed to get session details: " +
                         reply->errorString());
    }
    reply->deleteLater();
  });
