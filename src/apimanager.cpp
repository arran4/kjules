...
void APIManager::setNetworkAccessManager(QNetworkAccessManager *nam) {
  if (m_nam == nam) {
    return;
  }
  if (m_nam) {
...
void APIManager::reloadSession(const QString &sessionId, bool isBackground) {
  ...
  connect(reply, &QNetworkReply::finished, this, [this, reply, sessionId, isBackground]() {
    if (reply->error() == QNetworkReply::NoError) {
      QByteArray data = reply->readAll();
      QJsonDocument doc = QJsonDocument::fromJson(data);
      QString returnedId = doc.object().value(QStringLiteral("id")).toString();
      QString cleanId = cleanSessionId(sessionId);
      QString cleanReturnedId = cleanSessionId(returnedId);

      if (doc.isNull() || !doc.isObject() || returnedId.isEmpty() || cleanReturnedId != cleanId) {
