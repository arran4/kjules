...
ActivityBrowser::ActivityBrowser(QWidget *parent) : QTextBrowser(parent) {
  ...
  connect(this, &QTextBrowser::anchorClicked, this, &ActivityBrowser::onAnchorClicked);
  connect(this, &QWidget::customContextMenuRequested, this, &ActivityBrowser::onCustomContextMenu);
}
...
void ActivityBrowser::renderHtml() {
  ...
  if (!m_prompt.isEmpty()) {
      html += QStringLiteral("<div class='prompt'>") + i18n("Prompt") + QStringLiteral("</div><div style='margin-bottom: 10px; padding: 10px; background-color: #f0f0f0; border-radius: 5px;'>");
      ...
      if (!promptExpanded && lines.size() > 5) {
          html += QStringLiteral("<div style='white-space: pre-wrap; font-family: monospace;'>") + lines.mid(0, 5).join(QLatin1Char('\n')).toHtmlEscaped() + QStringLiteral("\n...</div>");
          html += QStringLiteral("<div style='margin-top: 10px;'><a href='expand:prompt_view' style='font-size: 0.9em; color: #3498db; text-decoration: none;'>") + i18n("Click to expand prompt") + QStringLiteral("</a></div>");
      } else if (promptExpanded && lines.size() > 5) {
          html += QStringLiteral("<div style='white-space: pre-wrap; font-family: monospace;'>") + m_prompt.toHtmlEscaped() + QStringLiteral("</div>");
          html += QStringLiteral("<div style='margin-top: 10px;'><a href='collapse:prompt_view' style='font-size: 0.9em; color: #3498db; text-decoration: none;'>") + i18n("Click to collapse prompt") + QStringLiteral("</a></div>");
      } else {
          html += QStringLiteral("<div style='white-space: pre-wrap; font-family: monospace;'>") + m_prompt.toHtmlEscaped() + QStringLiteral("</div>");
      }
  ...
  if (m_activities.isEmpty()) {
    html += QStringLiteral("<p><i>") + i18n("No activity feed available.") + QStringLiteral("</i></p>");
  } else {
    QDateTime lastTime;
    ...
    for (int i = 0; i < dedupedActivities.size(); ++i) {
      ...
      if (id.isEmpty()) id = QString::number(i);

      int count = repeatCounts[i];

      QJsonDocument doc(activity);
      m_activityJsons.insert(id, QString::fromUtf8(doc.toJson(QJsonDocument::Indented)));

      QString createTimeStr = activity.value(QStringLiteral("createTime")).toString();
      QDateTime createTime = QDateTime::fromString(createTimeStr, Qt::ISODateWithMs);
      if (!createTime.isValid()) createTime = QDateTime::fromString(createTimeStr, Qt::ISODate);

      if (lastTime.isValid() && createTime.isValid()) {
        qint64 secs = lastTime.secsTo(createTime);
        if (secs > 300) { // 5 minutes
          html += QStringLiteral("<div style='text-align:center; color:#999; margin: 10px 0;'>&mdash; ") +
                  i18n("%1 minutes passed", secs / 60) +
                  QStringLiteral(" &mdash;</div>");
        }
      }
      if (createTime.isValid()) lastTime = createTime;
      ...
      if (count > 1) {
          if (!expanded) {
              html += QStringLiteral("<div style='color: #888;'><i>") + i18n("Repeated %1 times", count) + QStringLiteral("</i></div>");
              html += QStringLiteral("<div style='margin-top: 10px;'><a href='expand:") + id + QStringLiteral("' style='color: #3498db; text-decoration: none;'>") + i18n("Show All") + QStringLiteral("</a></div>");
          } else {
              html += QStringLiteral("<div style='color: #888; margin-bottom: 10px;'><i>") + i18n("Repeated %1 times", count) + QStringLiteral("</i></div>");
              html += generateHtmlForActivity(activity, expanded);
              html += QStringLiteral("<div style='margin-top: 15px; border-top: 1px dashed #ccc; padding-top: 10px;'><a href='collapse:") + id + QStringLiteral("' style='color: #3498db; text-decoration: none;'>") + i18n("Hide Repeated Items") + QStringLiteral("</a></div>");
          }
      } else {
          html += generateHtmlForActivity(activity, expanded);
      }
...
QString ActivityBrowser::generateRawJsonHtml(const QJsonObject &activity, bool expanded) {
  QString id = activity.value(QStringLiteral("id")).toString();
  QJsonDocument doc(activity);
  ...
  if (!expanded) {
      ...
      if (lines.size() > 6) {
          rawJson = lines.mid(0, 6).join(QLatin1Char('\n')) + QStringLiteral("\n...");
          html += QStringLiteral("<div class='code'>") + rawJson.toHtmlEscaped() + QStringLiteral("</div>");
          html += QStringLiteral("<a href='expand:") + id + QStringLiteral("'>") + i18n("Show more") + QStringLiteral("</a>");
      } else {
          html += QStringLiteral("<div class='code'>") + rawJson.toHtmlEscaped() + QStringLiteral("</div>");
      }
  } else {
      html += QStringLiteral("<div class='code'>") + rawJson.toHtmlEscaped() + QStringLiteral("</div>");
      html += QStringLiteral("<a href='collapse:") + id + QStringLiteral("'>") + i18n("Show less") + QStringLiteral("</a>");
  }
  return html;
}

QString ActivityBrowser::generateHtmlForActivity(const QJsonObject &activity, bool expanded) {
  QString id = activity.value(QStringLiteral("id")).toString();
  QString html;
  ...
  if (activity.contains(QStringLiteral("planGenerated"))) {
    ...
    for (int i=0; i<actions.size(); ++i) {
        html += QStringLiteral("<div>") + actions[i].toObject().value(QStringLiteral("description")).toString().toHtmlEscaped() + QStringLiteral("</div>");
    }
    html += QStringLiteral("</div>");
    ...
    if (!prTitle.isEmpty()) {
        html += QStringLiteral("<div class='box'>");
        html += QStringLiteral("<div style='margin-bottom: 5px; font-weight: bold; font-size: 1.1em;'>") + prTitle.toHtmlEscaped() + QStringLiteral("</div>");
        if (!prHeadBranch.isEmpty() && !prBaseBranch.isEmpty()) {
            html += QStringLiteral("<div style='margin-bottom: 10px; font-family: monospace; color: #555; background-color: #f1f1f1; padding: 4px; border-radius: 4px; display: inline-block;'>") + prBaseBranch.toHtmlEscaped() + QStringLiteral(" &larr; ") + prHeadBranch.toHtmlEscaped() + QStringLiteral("</div>");
        }
        if (!prBody.isEmpty()) {
            html += QStringLiteral("<div style='white-space: pre-wrap; color: #444;'>") + prBody.toHtmlEscaped() + QStringLiteral("</div>");
        }
        html += QStringLiteral("</div>");
    }
    ...
    for (int i=0; i<artifacts.size(); ++i) {
      ...
      if (art.contains(QStringLiteral("changeSet"))) {
        ...
        html += QStringLiteral("<div class='box'>");
        html += QStringLiteral("<b>") + i18n("Commit:") + QStringLiteral("</b> <a href='https://github.com/") + source.toHtmlEscaped() + QStringLiteral("/commit/") + commitId.toHtmlEscaped() + QStringLiteral("'>") + commitId.toHtmlEscaped() + QStringLiteral("</a><br/>");

        QString sugg = gp.value(QStringLiteral("suggestedCommitMessage")).toString();
        if (!sugg.isEmpty()) {
            html += QStringLiteral("<div class='code'>") + sugg.toHtmlEscaped() + QStringLiteral("</div>");
        }

        QString patch = gp.value(QStringLiteral("unidiffPatch")).toString();
        if (!patch.isEmpty()) {
             html += QStringLiteral("<a href='diff:") + id + QStringLiteral("'>") + i18n("View Patch") + QStringLiteral("</a>");
        }
        html += QStringLiteral("</div>");
         ...
         if (mime.startsWith(QStringLiteral("image/"))) {
             ...
             html += QStringLiteral("<a href='img:") + id + QStringLiteral("'><img src='data:") + mime + QStringLiteral(";base64,") + b64 + QStringLiteral("' style='max-width:200px; max-height:200px; border: 1px solid #ccc;'/></a>");
         }
      } else {
    ...
    if (!content.isEmpty()) {
         html += QStringLiteral("<div class='content'>") + content.toHtmlEscaped() + QStringLiteral("</div>");
    } else {
        ...
        for (int i=0; i<m_activities.size(); ++i) {
             ...
             if (id == path) {
                 ...
                 for (int j=0; j<artifacts.size(); ++j) {
                      ...
                      if (art.contains(QStringLiteral("changeSet"))) {
                          diffText = art.value(QStringLiteral("changeSet")).toObject().value(QStringLiteral("gitPatch")).toObject().value(QStringLiteral("unidiffPatch")).toString();
                      }
                 }
             }
        }
             ...
             if (id == path) {
                 ...
                 for (int j=0; j<artifacts.size(); ++j) {
                      ...
                      if (art.contains(QStringLiteral("media"))) {
                          b64 = art.value(QStringLiteral("media")).toObject().value(QStringLiteral("data")).toString();
                      }
                 }
             }
        }
...
void ActivityBrowser::onCustomContextMenu(const QPoint &pos) {
    ...
    QString clickedId; // In a real browser we might map pos to block, but QTextBrowser doesn't make this trivial
    // Let's just provide a generic "Copy All JSON" for now, or if they right clicked near text we can't easily extract the ID.
    // However, QTextBrowser cursor gives us block.
