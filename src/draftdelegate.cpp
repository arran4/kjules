...
DraftDelegate::DraftDelegate(QObject *parent)
...
void DraftDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    QString automationMode = index.data(DraftsModel::AutomationModeRole).toString();
    // Assuming source is stored as list in new logic, but DraftsModel::SourceRole might return string or list.
    // In DraftsModel::data, SourceRole returns "source".
    // I should update DraftsModel to handle "sources" array.
    // For now, let's assume it returns whatever was stored.
    // If I updated NewSessionDialog to store "sources" array, DraftsModel::data(SourceRole) might return QVariant(QJsonArray) or QVariantList.
    // Let's check DraftsModel::data implementation.
